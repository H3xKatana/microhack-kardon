# Testing Guide for Kardon

This guide explains how to write tests for Kardon using our pytest-based testing strategy.
...
```python
import pytest
from kardon.api.serializers import MySerializer
...
Smoke tests should be placed in `tests/smoke/` directory and use the `kardon_server` fixture to test against a real HTTP server.
...
class TestCriticalFlow:
    @pytest.mark.django_db
    def test_login_flow(self, kardon_server, create_user, user_data):
        # Get login URL
        url = f"{kardon_server.url}/api/auth/signin/"
...
```python
from kardon.tests.factories import UserFactory, WorkspaceFactory
...
python -m pytest -m unit --cov=kardon
```
