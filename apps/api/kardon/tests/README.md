...
```bash
# Run unit tests
python -m pytest kardon/tests/unit/

# Run API contract tests
python -m pytest kardon/tests/contract/api/

# Run App contract tests
python -m pytest kardon/tests/contract/app/

# Run smoke tests
python -m pytest kardon/tests/smoke/
```
...
- `kardon_server`: Live Django test server for HTTP-based smoke tests
...
2. Use the correct client fixture based on the API being tested:
   ...
   - For smoke tests with real HTTP, use `kardon_server`
...
```bash
python -m pytest --cov=kardon --cov-report=term --cov-report=html
```
